...
def neuron(students_data, projects_data,students_score,student_id):
    ...
    unique_skills_students = sorted(set(skill for skills_list in students_data for skill in skills_list))
    students_skills = {skill: index for index, skill in enumerate(unique_skills_students)}

    unique_skills_projects = sorted(set(skill for skills_list in projects_data for skill in skills_list))
    ...
    students_data_vectors = np.array([skills_to_vector(student, universal_skills_mapping, score) for student, score in zip(students_data, students_score)])
    projects_data_vectors = np.array([skills_to_vector(project, universal_skills_mapping, 0) for project in projects_data])


    model = tf.keras.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_shape=(len(students_skills) + 1,)),
        ...
        with tf.GradientTape() as tape:
            ...
            for i in range(len(students_data)):
                ...
                for j in range(len(projects_data_vectors)):
                    projects_action = projects_data_vectors[j][:len(students_data_vectors[i])]
                    if len(projects_action) != len(students_data_vectors[i]):
                        diff_len = len(students_data_vectors[i]) - len(projects_action)
                        projects_action = np.append(projects_action, np.zeros(diff_len))

                    score = skill_match_reward(students_data_vectors[i], projects_action)
                ...
                if team_sizes[action] < max_team_size:
                    ...
                    if avg_team_score < weak_team_score:
                        ...
                        if team_sizes[weak_team_index] < max_team_size:
                            team_projects[weak_team_index].append(students_data[i])
                            team_sizes[weak_team_index] += 1
                      
                            team_scores[weak_team_index].append(students_score[i])
            ...
            # # Проверяем, есть ли еще нераспределенные студенты
            print(team_sizes)
            print(num_teams)
            print(team_projects)
            print(teamst)
            ...
            if unassigned_students:
                for student in unassigned_students:
                    eligible_teams = [team_idx for team_idx in range(num_teams) if team_sizes[team_idx] < max_team_size]
                    if eligible_teams:
                        weakest_team_index = min(eligible_teams, key=lambda k: np.mean(team_scores[k]))
                        student_skills = students_data[student]
                        team_projects[weakest_team_index].append(student_skills)
                        team_sizes[weakest_team_index] += 1
                        team_scores[weakest_team_index].append(students_score[student])
